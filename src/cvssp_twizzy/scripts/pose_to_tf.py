...
def pose_cb(pose_stamped):
    try:
        #This should be lookup_transform(target_frame,source_frame), but pose_stamped is sensor in world
        #IF we had pose_stamped as world in sensor cf, then the equation goes:
        # bPm = bTs * sPm
        # where
        # bPm is pose of map in base
        #
    ...
    #transform = t.concatenate_matrices(t.translation_matrix(trans.transform.translation),
    #                                   t.translation_matrix(trans.transform.rotation))
    #transform_inverse = t.inverse_matrix(transform)
    #trans = transform_inverse
